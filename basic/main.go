...
type Block struct {
	Index     int
	Timestamp string
	BPM       int	//beats per min,your pulse rate
	Hash      string
	PrevHash  string
}
...
func handleWriteBlock(w http.ResponseWriter, r *http.Request)  {
	...
	if isBlockValid(newBlock,Blockchain[len(Blockchain)-1]) {
		...
		//spew.Dump is a convenient function that pretty prints our structs into the console.
...
func replaceChain(newBlocks []Block) {
	...
	if len(newBlocks) > len(Blockchain) { Blockchain = newBlocks }
